...
static int init_test_buffers(const char *psname, uint32_t count) {
  ...
  rv |= BUFFER_RESIZE(pubkeybuf, minipkpsig_simple_get_publickey_bytes(psname));
  rv |= BUFFER_RESIZE(seckeybuf, minipkpsig_simple_get_secretkey_bytes(psname));
  rv |= BUFFER_RESIZE(msgbuf, 33 * (size_t)count);
  rv |= BUFFER_RESIZE(sigbuf, minipkpsig_simple_get_signature_bytes(psname));
...
static int compute_test_vector(const char *psname, uint32_t i) {
  ...
  rv = minipkpsig_simple_keypair(psname, pubkeybuf.data, seckeybuf.data);
  if (rv != 0) {
  ...
  rv = minipkpsig_simple_detached_verify(psname, sigbuf.data, msgbuf.data, msgbuf.len, pubkeybuf.data);
  if (rv != 0) {
...
int main(int argc, char *argv[]) {
  ...
  if (paramset_name == NULL) {
    minipkpsig_ps_enum_names(enum_names_cb, &count);
  } else {
    for (i = 0; i < count; ++i) {
